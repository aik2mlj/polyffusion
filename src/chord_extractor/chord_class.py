            ...
            for q in QUALITIES:
                ...
                if q in INVERSIONS:
                    for inv in INVERSIONS[q]:
                        ...
                        self.chord_list.append(name)
                        self.chroma_templates.append(np.roll(original_template, i))
                        self.bass_templates.append(
                            np.roll(BASS_TEMPLATE, i + delta_scale)
                        )
        self.chroma_templates = np.array(
            [list(entry) for entry in self.chroma_templates]
        )
    ...
    def score(self, chroma, basschroma):
        ...
        for i, c in enumerate(self.chord_list):
            if c == "N":
                ...
                score = (
                    (chroma[ref_chroma > 0].sum() - chroma[ref_chroma == 0].sum())
                    / (ref_chroma > 0).sum()
                    + 0.5 * basschroma[ref_bass_chroma > 0].sum()
                    - (ref_chroma > 0).sum() * 0.1
                    - ("/" in c) * 0.05
                )
                result[i] = score
        return result
    ...
    def batch_score(self, chromas, basschromas):
        ...
        for i, c in enumerate(self.chord_list):
            if c == "N":
                ...
                score = (
                    (
                        chromas[:, ref_chroma > 0].sum(axis=1)
                        - chromas[:, ref_chroma == 0].sum(axis=1)
                    )
                    / (ref_chroma > 0).sum()
                    + 0.5 * basschromas[:, ref_bass_chroma > 0].sum(axis=1)
                    - (ref_chroma > 0).sum() * 0.1
                    - ("/" in c) * 0.05
                )
                result[:, i] = score
