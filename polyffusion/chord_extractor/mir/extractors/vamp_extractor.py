...
from mir.common import PACKAGE_PATH, SONIC_ANNOTATOR_PATH, WORKING_PATH
...
class NNLSChroma(ExtractorBase):
    ...
    def extract(self, entry, **kwargs):
        ...
        temp_path = os.path.join(
            WORKING_PATH, "temp/nnlschroma_extractor_%s.wav" % hasher(entry.name)
        )
        temp_n3_path = temp_path + ".n3"
        rewrite_extract_n3(
            entry, os.path.join(PACKAGE_PATH, "data/bothchroma.n3"), temp_n3_path
        )
...
class TunedLogSpectrogram(ExtractorBase):
    ...
    def extract(self, entry, **kwargs):
        ...
        temp_path = os.path.join(
            WORKING_PATH,
            "temp/tunedlogspectrogram_extractor_%s.wav" % hasher(entry.name),
        )
        temp_n3_path = temp_path + ".n3"
        rewrite_extract_n3(
            entry, os.path.join(PACKAGE_PATH, "data/tunedlogfreqspec.n3"), temp_n3_path
        )
...
class GlobalTuning(ExtractorBase):
    ...
    def extract(self, entry, **kwargs):
        music_io = io.MusicIO()
        temp_path = os.path.join(
            WORKING_PATH, "temp/tuning_%s.wav" % hasher(entry.name)
        )
        temp_n3_path = temp_path + ".n3"
        rewrite_extract_n3(
            entry, os.path.join(PACKAGE_PATH, "data/tuning.n3"), temp_n3_path
        )
        if "source" in kwargs:
        ...
        result = (
            np.log2(np.float64(output[0].decode().split("\t")[2])) - np.log2(440)
        ) * 12
        try:
            os.unlink(temp_path)
            os.unlink(temp_n3_path)
