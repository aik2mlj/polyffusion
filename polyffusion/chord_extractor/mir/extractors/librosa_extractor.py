...
class STFT(ExtractorBase):
    ...
    def extract(self, entry, **kwargs):
        y = entry.music
        logspec = librosa.core.stft(
            y, win_length=kwargs["win_size"], hop_length=kwargs["hop_length"]
        ).T
        logspec = np.abs(logspec)
        return logspec
...
class Onset(ExtractorBase):
    ...
    def extract(self, entry, **kwargs):
        onset = librosa.onset.onset_strength(
            entry.music, sr=kwargs["sr"], hop_length=kwargs["hop_length"]
        ).reshape((-1, 1))
        return onset
