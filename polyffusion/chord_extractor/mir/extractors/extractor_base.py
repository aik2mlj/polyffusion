...
class ExtractorBase(ABC):
    ...
    def __create_cache_path(self, entry, cached_prop_record, input_kwargs):
        ...
        if len(items) == 0:
            ...
            folder_name = (
                ...
                + "/"
                + ",".join([k + "=" + str(items[k]) for k in sorted(items.keys())])
            )
        ...
        if len(items_entry) == 0:
            ...
            entry_name = (
                ...
                + "."
                + ",".join(
                    [k + "=" + str(items_entry[k]) for k in sorted(items_entry.keys())]
                )
                + ".cache"
            )
    ...
    def extract_and_cache(self, entry, cache_enabled=True, **kwargs):
        ...
        if (
            cache_enabled
            and entry.name != ""
            and self.get_feature_class() != io.UnknownIO
        ):
            # Need cache
            ...
            if cached_prop_record is None:
                ...
                cache_file_name = self.__create_cache_path(
                    entry, cached_prop_record, kwargs
                )
                need_io_create = True
            else:
                cache_file_name = self.__create_cache_path(
                ...
                if not os.path.isfile(cache_file_name):
                    ...
                    if cached_prop_record != new_prop_record:
                        print(
                            "[Warning] Inconsistent cached properity requirement in %s, overrode:"
                            % self.__class__.__name__
                        )
