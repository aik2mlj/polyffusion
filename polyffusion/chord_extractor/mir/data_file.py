...
class ProxyBase(ABC):
    ...
    def save_visualize_temp_file(
        self, requestor, savepath, auralize, override_sr, beats=None
    ):
        io = self.feature_class()
        if auralize:
...
class ExtractorProxy(ProxyBase):
    ...
    def load(self, requester):
        return self.extractor.extract_and_cache(
            requester, self.cache_enabled, **self.kwargs
        )
...
class DataEntryProperties:
    ...
    def set(self, item, value):
        ...
        elif item in self.dict and not isinstance(
            self.dict[item], mir.io.LoadingPlaceholder
        ):
            # Old value found
            if self.dict[item] != value:
                print(
                    "Warning: Inconsistant property in %s: old value" % item,
                    self.dict[item],
                    "new value",
                    value,
                )
...
class DataEntry:
    ...
    def append_file(self, filename, feature_class, output_name, file_exist_check=True):
        file_proxy = FileProxy(
            filename, feature_class, file_exist_check=file_exist_check
        )
        file_proxy.pre_assign(self)
        self.dict[output_name] = file_proxy

    def apply_extractor(
        self, extractor_class, cache_enabled=True, io_override=None, **kwargs
    ):
    ...
    def activate_proxy(
        self, item, free=False, verbose_id=0, verbose_all=0, start_time=None
    ):
        if verbose_all > 0:
            if verbose_id > 0 and start_time is not None:
                current_time = time.time()
                print(
                    "[%d/%d]Activating %s, passed:" % (verbose_id, verbose_all, item),
                    str(datetime.timedelta(seconds=current_time - start_time)),
                    "remaining:",
                    str(
                        datetime.timedelta(
                            seconds=(current_time - start_time)
                            / verbose_id
                            * (verbose_all - verbose_id)
                        )
                    ),
                    flush=True,
                )
                ...
                print(
                    "[%d/%d]Activating %s" % (verbose_id, verbose_all, item), flush=True
                )
    ...
    def visualize(
        self,
        items=None,
        use_raw_music_file=True,
        music="music",
        midi_texture_builder: TextureBuilder = None,
    ):
        if items == None:
            items = []
            ...
            for music_item in music_list:
                if not isinstance(self.dict[music_item], FileProxy):
                    ...
                    filepath = os.path.join(
                        WORKING_PATH, self.dict[music_item].filepath
                    )
        ...
        for item in items:
            if self.has(item):
                abbr = (
                    "_visualize."
                    + self.dict[item].feature_class().get_visualize_extention_name()
                )
                ...
                self.dict[item].save_visualize_temp_file(
                    self, temp_file_name, auralize=False, override_sr=override_sr
                )
            else:
        ...
        if midi_texture_builder is not None:
            ...
            if self.has(chords_item):
                ...
                generator.auralize(
                    ...
                    chords=self.dict[chords_item].get(self),
                    beats=beats,
                    sr=sr,
                    win_shift=win_shift,
                )
...
class DataPool:
    ...
    def add_entry(self, entry):
        ...
        if "&" in self.name:
            ...
            print(
                "Warning: Inconsistent dataset name, %s expected, %s found"
                % (self.name, entry.name.split("/")[0])
            )
    ...
    def new_entry(self, filename):
        if "&" in self.name:
            print(
                "Warning: You are creating an entry in a joint dataset. Don't do that!"
            )
    ...
    def append_folder(
        self, folder_path, suffix, typename, output_name, recursive=False
    ):
        if recursive:
            files = [
        ...
        if len(self.dict) == 0:
            for file in files:
                if file.endswith(suffix):
                    ...
                    entry = DataEntry(self.name + "/" + filename)
                    entry.append_file(
                        file, typename, output_name=output_name, file_exist_check=False
                    )
                    for k in self.default_prop:
                        entry.prop.set(k, self.default_prop[k])
                    self.__append_entry(entry, filename)
    ...
    def activate_proxy(self, item, thread_number=1, timing=True, free=False):
        ...
        if thread_number != 1:
            ...
            Parallel(n_jobs=thread_number)(
                delayed(DataEntry.activate_proxy)(
                    entries_needs[i], item, free, i, total, start_time
                )
