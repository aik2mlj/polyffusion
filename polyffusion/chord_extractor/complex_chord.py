...
def enum_to_list(cls, valid_only):
    items = [
        ...
        if not item[0].startswith("_") and (not valid_only or item[1] >= 0)
    ]
...
def enum_to_dict(cls):
    return {
        item[1]: item[0] for item in cls.__dict__.items() if not item[0].startswith("_")
    }
...
class ChordTypeLimit:
    def __init__(
        self, triad_limit, seventh_limit, ninth_limit, eleventh_limit, thirteenth_limit
    ):
        ...
        self.ninth_slice_begin = self.seventh_slice_begin + 12 * (
            self.seventh_limit + 1
        )
        self.eleventh_slice_begin = self.ninth_slice_begin + 12 * (self.ninth_limit + 1)
        self.thirteenth_slice_begin = self.eleventh_slice_begin + 12 * (
            self.eleventh_limit + 1
        )
...
def shift_complex_chord_array_list(array, shift):
    ...
    new_array[bass_valid_indices, 1] = (
        (new_array[bass_valid_indices, 1] + shift) % 12 + 12
    ) % 12
    return new_array
...
def create_tag_list(chord_limit):
    ...
    result += [
        "%s" % (thirteenth_dict[i]) for i in range(chord_limit.thirteenth_limit + 1)
    ]
    return result
