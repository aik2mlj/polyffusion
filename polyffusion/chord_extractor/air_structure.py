...
class AIRStructure:
    ...
    def __overwrite_warning(self, pos, error_item, time):
        if pos == 0:
            self.log_error(
                "Warning: Too early %s event happened at time %.2f" % (error_item, time)
            )
            return True  # allow overwrite
        elif pos == self.length - 1:
            self.log_error(
                "Warning: Too late %s event happened at time %.2f" % (error_item, time)
            )
            return False  # disallow overwrite
        else:
            self.log_error(
                "Warning: Overwrite %s event at time %.2f. Music is probably too fast."
                % (error_item, time)
            )
            return False  # disallow overwrite
    ...
    def export_to_array(self, export_all=False):
        ...
        (
            ...
            export_chord_chroma,
            export_chord_bass,
        ) = mir_eval.chord.encode_many(chord)
        export_chord_chroma = mir_eval.chord.rotate_bitmaps_to_roots(
            export_chord_chroma, export_chord_root
        )
        ...
        export_key = (
            self.key[valid].reshape((export_length, 1))
            if "key" in self.__dict__
            else np.ones((export_length, 1), dtype=np.int32) * -1
        )
        return np.hstack((export_start, export_end)), np.hstack(
            (
                export_melody,
                export_melody_onset,
                export_chord_root,
    ...
    def __init_timing_and_offset(self):
        ...
        for i in range(self.input_beat.shape[0] - 1):
            ...
            timing[
                i * self.num_beat_division : (i + 1) * self.num_beat_division + 1
            ] = np.linspace(cur_time, next_time, self.num_beat_division + 1)
            offset[
                i * self.num_beat_division : (i + 1) * self.num_beat_division
            ] = np.round(self.input_beat[i, 1])
        ...
        for pos in range(self.pos_start, self.pos_end):
            ...
            for type in range(3):  # for type in ['melody','lyric','chord']
                ...
                if type == 0:  # melody
                    ...
                    if (
                        pos == 0
                        or air.chord[pos] != air.chord[pos - 1]
...
class PhraseRenderer:
    ...
    def render(self, unit_width, bar_group):
        data = np.full(
            (len(self.bars), bar_group, OVERFLOW_WIDTH_LIMIT, 3), "", dtype="<U1"
        )
        air = self.air
        for i in range(len(self.bars)):
            ...
            for j in range(bar_group):
                data[i, j, :, :] = UnitRenderer(
                    air, int(interp[j]), int(interp[j + 1])
    ...
    def to_string(self, air: AIRStructure, unit_width, bar_group):
        ...
        if air.phrase_count == 0:
            ...
            for phrase_id in range(air.phrase_count):
                result += (
                    PhraseRenderer(air, phrase_id).to_string(unit_width, bar_group)
                    + "\n"
                )
