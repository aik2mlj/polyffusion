...
class ChordRecognition:
    ...
    def process_feature(self, channel_weights):
        ...
        for instrument in midi.instruments:
            ...
            for note in raw_notes:
                ...
                for j in range(left_subbeat, right_subbeat):
                    min_subbeat_bass[j] = min(
                        min_subbeat_bass[j], note.pitch
                    )  # todo: weighted bass
                for j in range(left_beat, right_beat):
                    beat_chroma[j][note.pitch % 12] = max(
                        beat_chroma[j][note.pitch % 12],
        ...
        for i in range(SUBBEAT_COUNT):
            ...
            beat_bass[valid_indices, update_terms[valid_indices] % 12] += (
                1.0 / SUBBEAT_COUNT
            )
    ...
    def decode(self):
        ...
        batch_score = self.chord_class.batch_score(
            batch_chroma.reshape((-1, 12)), batch_bass_chroma.reshape((-1, 12))
        ...
        for i in range(n_frame):
            for j in range(MAX_PREV):
                ...
                obs[i, j, :] = (
                    batch_score[i, j]
                    + j * 0.7
                    + self.is_halfdownbeat[i - j] * 0.15
                    + self.is_even_beat[i - j] * 0.2
                )
        ...
        while current_i >= 0:
            ...
            start = (
                ...
                current_i
                if self.half_beat_switch
                or current_i == n_frame - 1
                or self.is_even_beat[current_i + 1]
                else current_i + 1
            )
            result.append(
                [
                    self.qt_beat_onset[start],
                    self.qt_beat_offset[end],
                    self.chord_class.chord_list[prev_c],
                ]
            )
            current_i = prev_i
        result = result[::-1]
