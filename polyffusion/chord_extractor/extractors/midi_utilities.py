...
from pretty_midi import PitchBend, pitch_bend_to_semitones
...
def get_pretty_midi_energy_roll(midi, fs=100, times=None):
    ...
    piano_rolls = [get_energy_roll(i, fs=fs, times=times) for i in midi.instruments]
    # Allocate piano roll,
...
def get_energy_roll(self, fs=100, times=None):
    ...
    times = np.array(times * fs, dtype=int)
...
def is_percussive_channel(instrument):
    return (
        instrument.is_drum or instrument.program > 112
    )  # todo: are >112 instruments all percussive?
